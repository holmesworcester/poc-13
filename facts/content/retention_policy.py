...
as an ordinary validated Provide: the query takes the latest (ts, owner) row,
...
from kernel import (Atom, Exact, PROVIDE, Out, REQUIRE, Range, SELF, by,
...
def policy(workspace_id, ttl, t):
    return fact(TAG, ts_atom(t, workspace_id),
                ...
                Atom(REQUIRE, b"admin", workspace_id, IDS),
                Atom(PROVIDE, b"retention", workspace_id, Exact(b"window"),
...
def project(f, ctx):
    try:
        r = next(a for a in f.atoms if a.name == b"retention")
...
def set_window(node, workspace_id, ttl, t):
    ...
    admins = {a.target[0] for _, _, a in node.provided(b"admin", workspace_id)}
    def build(member_id):
...
def window(node, workspace_id):                       # the latest (ts, owner) row wins
    ...
    row = max(node.provided(b"retention", workspace_id), key=lambda r: (r[1], r[0]), default=None)
