...
membership Provide, so a grant can never outrun the membership it elevates, and
...
def admin(workspace_id, user_id, t):
    return fact(TAG, ts_atom(t, workspace_id),
                Atom(REQUIRE, b"member", workspace_id, Exact(user_id)),
                Atom(REQUIRE, b"root", b"auth", Exact(workspace_id)),
                Atom(REQUIRE, b"pk", workspace_id, SELF),
                Atom(PROVIDE, b"admin", workspace_id, Exact(user_id)))
...
def project(f, ctx):                 # the granter's signer key must be the founder root
    ...
    return Out(provides=tuple(a for a in f.atoms if a.name == b"admin") + (sync_leaf(),))
...
def admins(node, workspace_id):
    ...
    return [a.target[1] for o, t, a in sorted(node.provided(b"admin", workspace_id),
