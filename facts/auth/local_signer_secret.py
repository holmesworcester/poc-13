...
def project(f, ctx):
    return Out(provides=tuple(a for a in f.atoms if a.name in (b"sk", b"pk")))
...
def current(node):
    ...
    sk = next((a.value for _, _, a in node.provided(b"sk", b"local")), None)
    pk = next((a.value for _, _, a in node.provided(b"pk", b"local")), None)
