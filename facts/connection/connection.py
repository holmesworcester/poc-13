...
`conn_secret` Provide (never synced, rebuilt by replay), and `answered` to retire
...
def project(f, ctx):
    env = next(a.value for a in f.atoms if a.name == b"sconn")
    ...
    return Out(provides=(Atom(PROVIDE, b"connection", SC, SELF, frame(peer_ep, peer_addr)),
                       Atom(PROVIDE, b"conn_secret", SC, SELF, cs),
                       Atom(PROVIDE, b"answered", SC, Exact(request_id))))
...
def _for_request(node, request_id):
    return next((o for o, _, a in node.provided(b"answered", SC)
...
def peers(node):                         # (peer endpoint, addr, connection id, who) per live session
    ...
    out = []
    for o, _, a in node.provided(b"connection", SC):
...
def _bindings(node):                     # endpoint pk -> the member it belongs to (auth), via device facts
    out = {}
    for _, _, a in node.provided(b"endpoint_shared", b"auth"):
        ep, spk, wid = unframe(a.value)
        uid = next((o for o, _, k in node.provided(b"key", wid) if k.value == spk), None)
        name = next((m.value for o, _, m in node.provided(b"member", wid) if o == uid), None)
...
def _valid(node, name, target):          # clean rows matching an exact target
    return [r for r in node.clean.get((name, SC), ()) if r[2].target == target]

def _invite_secret(node, bh):
    return next((a.value for _, _, a in node.provided(b"invite_secret", b"local")
