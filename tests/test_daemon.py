...
def test_proxy_roundtrip_and_restart():
    with tempfile.TemporaryDirectory() as d:
        ...
        try:
            ...
            assert con(db, "chat.note.send", "general", "hi", "1") == fid  # idempotent via proxy
        ...
        try: assert con(db, "chat.note.feed", "general") == "hi"
        finally: stop(p)
...
def test_two_daemons_fact_travels():
    with tempfile.TemporaryDirectory() as d:
        ...
        try:
            con(dba, "chat.note.send", "general", "over the wire", "1")
            assert until(lambda: con(dbb, "chat.note.feed", "general") == "over the wire")
...
def test_tombstone_travels():
    with tempfile.TemporaryDirectory() as d:
        ...
        pa, _ = spawn(dba, "--peer", addr)
        try:
            doomed = con(dba, "chat.note.send", "general", "doomed", "1")
            con(dba, "chat.note.send", "general", "keep", "2")
            assert until(lambda: con(dbb, "chat.note.feed", "general") == "doomed\nkeep")
            con(dba, "chat.tombstone.delete", "general", doomed, "3")
            assert until(lambda: con(dbb, "chat.note.feed", "general") == "keep")
        finally: stop(pa); stop(pb)
...
def test_slow_and_absent_peers_never_wedge():
    with tempfile.TemporaryDirectory() as d:
        ...
        try:
            for i in range(20): con(db, "chat.note.send", "general", "m%d" % i, str(i + 1))
            assert len(con(db, "chat.note.feed", "general").splitlines()) == 20
