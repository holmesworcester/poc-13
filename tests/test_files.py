...
from kernel import Exact, NEED, OFFER, SUPPRESS, Node, Store, encode, fact_id
from facts import ROOT
from facts.auth import workspace
from facts.content import file, file_slice, message, message_deletion
...
def _node():
    n = Node(ROOT); wid = workspace.create(n, b"acme", 1); n.run(); return n, wid
...
def test_send_view_save_roundtrip_atomized_descriptor_and_cold_hydration():
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        assert len(descriptor.atoms) == 11
...
def test_only_valid_bao_slices_count_and_incomplete_save_is_atomic():
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        for item in (parent, descriptor, first): assert n.admit(encode(item)) is not None
        n.run()
