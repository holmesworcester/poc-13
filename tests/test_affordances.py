...
CHANNEL = channel(WID, b"g", T0 + 1); CHANNEL_ID = fact_id(CHANNEL)
...
def test_closure_includes_self_and_spine():
    m = message(WID, CHANNEL_ID, b"al", b"hi", T0 + HOUR); mid = fact_id(m)
    c = node(WS, WS_SIG, CHANNEL, m).closure(mid)
    assert {mid, CHANNEL_ID, WID, fact_id(WS_SIG)} <= c     # message -> channel -> workspace spine

def test_deps_structural():
    m = message(WID, CHANNEL_ID, b"al", b"hi", T0 + HOUR)
    n = node(WS, WS_SIG, CHANNEL, m)
...
def test_summary_small_range_is_one_id_list_with_closure():
    m = message(WID, CHANNEL_ID, b"al", b"hi", T0 + HOUR); mid = fact_id(m)
    n = node(WS, WS_SIG, CHANNEL, m)                        # 4 shareable leaves (<= T): whole domain is "small"
    ...
    assert {mid, CHANNEL_ID, WID, fact_id(WS_SIG)} <= ids  # channel is a real closure edge, by id, deduped

def test_summary_large_range_splits_by_equal_count():
    msgs = [message(WID, CHANNEL_ID, b"al", b"m%d" % i, T0 + HOUR + i * MIN) for i in range(40)]
    n = node(WS, WS_SIG, CHANNEL, *msgs)                   # > T leaves: the domain must be split, not listed
    rows = n._answer(summary_need(b"", HI))
