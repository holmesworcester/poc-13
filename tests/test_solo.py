...
def test_solo_story():
    with tempfile.TemporaryDirectory() as d, fleet() as f:
        ...
        general = tiny(db, "content.channel.id", wid, "general")
        assert len(general) == 64
        random = tiny(db, "content.channel.create", wid, "random", "2")
        listing = tiny(db, "content.channel.list", wid).splitlines()
        assert listing == [f"{general} general", f"{random} random"]
        # idempotent authorship and channel-isolated feeds through the proxy
        m_hi = tiny(db, "content.message.send", wid, "general", "al", "hi", "2")
        assert tiny(db, "content.message.send", wid, "general", "al", "hi", "2") == m_hi, \
            "resending the same fact must return the same id"
        tiny(db, "content.message.send", wid, "general", "bo", "there", "3")
        tiny(db, "content.message.send", wid, "random", "al", "elsewhere", "4")
        ...
        ghost = "00" * 32
        r = subprocess.run([sys.executable, os.path.join(BIN, "tiny.py"), db,
        ...
        tiny(db, "content.reaction.react", wid, m_hi, ":+1:", "7")
        converge(db, ":+1: founder", "content.reaction.on", wid, m_hi, secs=0, phase="reaction lands")
        tiny(db, "content.message_deletion.delete", wid, m_hi, "8")
        ...
        tiny(db, "content.retention_policy.set", wid, "1440", "9")
        ...
        m_warm = tiny(db, "content.message.send", wid, "general", "al", "warm", "12")
        assert tiny(db, "content.message.send", wid, "general", "al", "warm", "12") == m_warm, \
