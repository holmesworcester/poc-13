"""Two real daemons over a SEALED connection, black box through tiny.py. B
...
import os, tempfile
from harness import tiny, converge, fleet, port

def _bootstrap(dba, dbb, wid, addr_a, addr_b):
    link = tiny(dba, "auth.user_invite.invite", wid)
    iid, secret = link.split(":")
    ep = tiny(dba, "auth.endpoint.endpoint")
    tiny(dbb, "connection.request.connect", wid, iid, secret, ep, addr_a, addr_b)
    return link
...
def test_answered_request_redials_after_responder_restart():
    ...
    with tempfile.TemporaryDirectory() as d, fleet() as f:
        ...
        wid = tiny(dba, "auth.workspace.create", "acme", "1")
        link = _bootstrap(dba, dbb, wid, A, B)
        tiny(dbb, "auth.user.join", wid, "bo", link, "5")
