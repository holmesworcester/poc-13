...
def test_tombstone_stays_a_leaf_and_invalid_leaves():
    # Suppressed: the verdict the hook must see even though project() never runs.
    def project(f, ctx):
    ...
    assert n.memo[fid] == "Suppressed"
    assert leafkeys(n) and sidx.ver(n) == v0          # a tombstone: still a leaf, no delta
    # Invalid: a poison offer flips the verdict inside project — the minus path.
    f2 = toy(b"toy.l", 7, Atom(NEED, b"poison", b"t", SELF, effect=WATCH))
    fid2 = n.admit(encode(f2)); n.run()
    assert len(leafkeys(n)) == 2
    n.admit(encode(toy(b"toy.k", 8, Atom(OFFER, b"poison", b"t", Exact(fid2))))); n.run()
    assert n.memo[fid2] == "Invalid" and len(leafkeys(n)) == 1   # the leaf left the set
